...
def smooth_curve_average(points, sample_size):
    ...
    for i in range(0,len(points), sample_size):
        subconjunto = points[i:i+sample_size]
...
while(1):
    ...
    if(len(hrValues)==100):
            ...
            finalhr=analisisHR2(hrValues, miliValues, 1)
            ...
            try:
                ...
                insertVariablesIntoUser(1, nombres, apellidos, edad) 
                ...
                date = date.fromordinal(random.randint(start_date, end_date)) 
                oxygen = np.random.randint(75, 100)
                rythm = finalhr
                if oxygen < 90 and rythm < 60:
                    covid =1
                else:
                    covid=0
                insertVariblesIntoTable(1,1,oxygen, rythm, covid, date)
